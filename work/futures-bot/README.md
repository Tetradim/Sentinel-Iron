...
Strategy position targets can be converted to order intents through `futures_bot.application.order_planning.plan_order_to_target` or `plan_orders_to_targets`. Batch planning skips targets already at the current quantity, rejects duplicate targets and missing current positions, and emits risk-reducing orders before new exposure while preserving target order inside each group. The resulting intents still flow through readiness, risk, submission, broker, lifecycle, and audit services.
